...
/* CSCI 5708 (Jason Nowotny)
* Print the Free List wherever needed (for debugging purposes)
*/
...
static void PrintFreeList(){
	...
	if(StrategyControl->firstFreeBuffer == FREENEXT_NOT_IN_LIST){
		printf("Candidate Buffers: <freelist is empty>\n");
		return;
	}
...
StrategyGetBuffer(BufferAccessStrategy strategy, uint32 *buf_state)
{
	...
	for (;;)
	{
		...
		*/
		if(strategy == NULL){
			buf = &BufferDescriptors[StrategyControl -> MRUhead].bufferdesc;
		} else {
			//buf = &BufferDescriptors[StrategyControl -> nextVictimBuffer].bufferdesc; 
		}
		/*
		...
		else if (--trycounter == 0)
		{
			 ...
			 */
			 ...
			 /*
			* CSCI 5708 (Jason Nowotny)
			* If the buffer is unpinned unlink the buffer, and print free list
			*/
